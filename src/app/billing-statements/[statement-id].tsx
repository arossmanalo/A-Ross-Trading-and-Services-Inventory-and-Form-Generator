...
export default function BillingStatementDetailScreen() {
  ...
  const saveDraftValues = useCallback(async () => {
  ...
  }, [backdateReason, businessDate, db, discountType, discountValue, statementId]);

  const save = useCallback(() => run(saveDraftValues), [run, saveDraftValues]);
  ...
  const previewDraft = useCallback(async () => {
    ...
    try {
      ...
      router.push({ pathname: '/billing-statements/preview', params: { statementId } });
    } catch (previewError) {
      setError(previewError instanceof Error ? previewError.message : 'Could not save the statement before previewing.');
    } finally {
      setBusy(false);
    }
  }, [saveDraftValues, statementId]);
  ...
  const linkedCsrIsDraft = Boolean(statement.serviceReportId && !statement.serviceReportNumber);
  ...
  return <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={styles.content}>
    ...
    <View style={styles.card}><Label label="Customer" value={statement.customerName} /><Label label="Linked CSR" value={statement.serviceReportId ? statement.serviceReportNumber ?? 'Draft — unnumbered' : 'None'} /><Label label="Date" value={statement.businessDate} /></View>
    {!draft ? <View style={styles.card}><Label label="Signature status" value={statement.signatureStatus.replaceAll('_',' ')} />{statement.documentState === 'finalized' && statement.hasSignedVersion ? <ActionButton variant="secondary" onPress={() => router.push({ pathname: '/signatures/manage', params: { ownerType: 'billing_statement', ownerId: statementId, mode: 'preview' } })}>Preview signed version</ActionButton> : null}</View> : null}
    {draft && linkedCsrIsDraft && statement.serviceReportId ? <View style={styles.card}><Text selectable style={styles.help}>Billable items and services saved on the CSR appear here and stay synchronized while it is a draft. Edit those charges on the CSR, then finalize the CSR before finalizing this statement.</Text><ActionButton disabled={busy} variant="secondary" onPress={() => router.push({ pathname: '/service-reports/[report-id]', params: { 'report-id': statement.serviceReportId! } })}>Continue linked CSR draft</ActionButton></View> : null}
    {draft && linkedCsrIsDraft && statement.lines.length === 0 ? <View style={styles.card}><Text selectable style={styles.help}>No eligible billable CSR charges are available yet. Non-billable item usage is excluded, and a CSR source charge cannot be billed on more than one statement.</Text></View> : null}
