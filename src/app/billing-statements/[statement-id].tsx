...
export default function BillingStatementDetailScreen() {
  ...
  return <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={styles.content}>
    ...
    {draft && statement.serviceReportId && !statement.serviceReportNumber ? <View style={styles.card}><Text selectable style={styles.help}>This Billing Statement is linked to a CSR draft. Finalize the CSR before adding its item usage or finalizing this statement.</Text><ActionButton disabled={busy} variant="secondary" onPress={() => router.push({ pathname: '/service-reports/[report-id]', params: { 'report-id': statement.serviceReportId! } })}>Continue linked CSR draft</ActionButton></View> : null}
