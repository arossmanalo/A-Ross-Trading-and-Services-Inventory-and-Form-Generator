...
export async function finalizeBillingStatement(db: SQLiteDatabase, statementId: string, pricePolicy: 'keep-draft' | 'reject' | 'use-current' = 'reject'): Promise<{ bsNumber: string; html: string; snapshot: BillingStatementRenderSnapshot }> {
  let result: { bsNumber: string; html: string; snapshot: BillingStatementRenderSnapshot } | null = null;
  await db.withExclusiveTransactionAsync(async (tx) => {
    ...
    await tx.runAsync(`UPDATE billing_statements SET bs_number=?,document_state='finalized',subtotal_centavos=?,discounted_total_centavos=?,payment_choice='pay_later',vat_snapshot_json=?,content_snapshot_json=?,render_template_snapshot=?,template_version=?,pdf_state='pending',finalized_at=? WHERE id=? AND document_state='draft'`,bsNumber,subtotal,total,JSON.stringify({mode:row.vat_display_mode,rateBasisPoints:row.vat_rate_basis_points}),JSON.stringify(snapshot),html,BILLING_STATEMENT_TEMPLATE_VERSION,now,statementId);
    await auditAndRevise(tx,'billing_statement.finalized',statementId,{bsNumber,lineCount:lines.length,pricePolicy},now); result={bsNumber,html,snapshot};
  });
