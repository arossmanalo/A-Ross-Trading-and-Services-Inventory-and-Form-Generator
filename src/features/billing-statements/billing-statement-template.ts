...
export const BILLING_STATEMENT_TEMPLATE_VERSION = 'billing-a4-v3';
...
export function buildBillingStatementHtml(snapshot: BillingStatementRenderSnapshot): string {
  ...
  </style></head><body><header class="header"><div class="mark">${logoHtml(snapshot.business.logoDataUrl)}</div><div class="business"><div class="business-name">${escapeHtml(snapshot.business.name)}</div><div class="muted">${escapeHtml([snapshot.business.address,snapshot.business.contactDetails].filter(Boolean).join('\n'))}</div></div></header><h1 class="title">Billing Statement — Not a Tax Invoice</h1><section class="meta"><div class="meta-box"><span class="label">Client</span><span class="value">${escapeHtml(snapshot.customer.name)}\n${escapeHtml(snapshot.customer.address)}</span></div><div class="meta-box"><span class="label">Billing Statement</span><span class="value">${escapeHtml(snapshot.bsNumber)}\nDate ${escapeHtml(snapshot.businessDate)}${snapshot.serviceReportNumber ? `\nCSR ${escapeHtml(snapshot.serviceReportNumber)}` : ''}</span></div></section><table><thead><tr><th>Description</th><th class="number">Quantity</th><th class="number">Unit Price</th><th class="number">Amount</th></tr></thead><tbody>${rows}</tbody></table><section class="totals"><div class="total-row"><span>Subtotal</span><span>${money(snapshot.subtotalCentavos)}</span></div>${snapshot.discountCentavos ? `<div class="total-row"><span>${escapeHtml(snapshot.discountLabel ?? 'Discount')}</span><span>− ${money(snapshot.discountCentavos)}</span></div>` : ''}<div class="total-row"><span>Total</span><span>${money(snapshot.totalCentavos)}</span></div>${snapshot.paymentsReceivedCentavos ? `<div class="total-row"><span>Payments Received</span><span>− ${money(snapshot.paymentsReceivedCentavos)}</span></div>` : ''}<div class="total-row grand"><span>Balance Due</span><span>${money(snapshot.balanceDueCentavos)}</span></div><div class="vat">${escapeHtml(vat)}</div></section><footer class="footer"><div class="disclaimer">This is a billing statement, not a tax invoice. All prices are VAT ${snapshot.vatDisplayMode === 'exclusive' ? 'exclusive' : snapshot.vatDisplayMode === 'inclusive' ? 'inclusive' : 'not VAT-classified'}.</div><div class="fingerprint">${escapeHtml(snapshot.bsNumber)} · Revision 1 · ${BILLING_STATEMENT_TEMPLATE_VERSION} · ${escapeHtml(snapshot.fingerprint)}</div></footer></body></html>`;
  const signatureSection = '<section class="signatures"><div class="signature" data-signature-role="preparer"><div class="signature-writing" data-signature-image-slot="preparer"></div><div class="signature-line"></div><div class="signature-name" data-signature-name-slot="preparer"></div><div class="signature-label">Prepared By</div></div><div class="signature" data-signature-role="customer"><div class="signature-writing" data-signature-image-slot="customer"></div><div class="signature-line"></div><div class="signature-name" data-signature-name-slot="customer"></div><div class="signature-label">Customer</div></div></section>';
  const withSignatures = html.replace('<footer class="footer">', `${signatureSection}<footer class="footer">`)
    .replace('</style>', 'body{overflow-wrap:anywhere}tr{break-inside:avoid}thead{display:table-header-group}</style>');
  return applySignatureCapturesToDocument(withSignatures, [snapshot.preparerSignatureHtml ?? '']);
}
