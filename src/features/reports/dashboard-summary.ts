...
import { getBackupStatus, type BackupStatus } from '@/features/backup/backup-repository';
...
export async function getDashboardSummary(db: SQLiteDatabase): Promise<DashboardSummary> {
  const [items, lowStock, customers, backupStatus, recentActivity] = await Promise.all([
