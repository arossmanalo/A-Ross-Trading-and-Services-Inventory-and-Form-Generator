...
describe('database migrations', () => {
  ...
  it('migrates a new database through version 8 with default metadata', async () => {
  ...
  it('upgrades a version 1 database with the later snapshot and signature schema', async () => {
    ...
    const statementLineColumns = raw.prepare("PRAGMA table_info('billing_statement_lines')").all() as Array<{ name: string }>;
    expect(statementLineColumns.map(column => column.name)).toContain('source_csr_service_usage_id');
