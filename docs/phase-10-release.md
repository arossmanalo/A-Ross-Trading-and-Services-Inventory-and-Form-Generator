...
| UAT-04 Tablet layout | Repeat UAT-03 on the owner tablet. Compare Dashboard, forms, lists, and a long document with the phone. | Content uses the full tablet viewport with consistent internal padding; controls remain readable, unclipped, and reachable without an isolated narrow column. | |
...
| UAT-12 CSR charges/finalization | In a new CSR draft, add Item A quantity `1` as billable, Item B quantity `1` as non-billable, and `QA Labor` as a service. Confirm the read-only Total Bill equals Item A's price plus the service rate; open each picker and use its **Add new** action if a fixture is missing. Finalize, then tap finalization twice quickly. | Non-billable Item B is excluded from the total; the total updates after adding/removing a service or item; new catalog entries return to the picker; exactly one next `CSR-` number is allocated and one unit of each item is deducted exactly once. | |
| UAT-13 CSR PDF | From the finalized CSR choose **Preview PDF**, scroll through the frozen document, then choose Generate/Retry PDF and open/share it. | The offline preview shows the same finalized customer/equipment snapshots, number, date, sections, total, and signatures; the generated PDF is portrait Legal size and remains legible. | |
| UAT-13b Billing Statement preview | From a finalized Billing Statement choose **Preview Billing Statement**, scroll through the frozen document, then generate/share the statement PDF. | The offline preview shows the same customer, number, date, charges, discount, payments, balance, and signatures as the generated portrait A4 PDF. | |
...
| UAT-33 Business visual sign-off | Compare CSR, Billing Statement, and Payment Acknowledgment PDFs with the approved sample formats. Verify the bundled A.Ross logo (or an owner-selected replacement), address, contacts, titles, disclaimers, and VAT mode. | Owner approves spacing, typography, page size, logo/header, totals, signatures, and the non-tax-document wording. | |
