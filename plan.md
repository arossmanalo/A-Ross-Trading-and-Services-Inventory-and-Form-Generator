...
4. Optionally start a linked Billing Statement draft from the CSR draft. Existing billable item usages and service usages appear automatically, and later CSR draft usage changes synchronize to the linked statement. The statement remains unnumbered and cannot be finalized until this CSR is finalized. Deleting the CSR draft also removes its linked unnumbered Billing Statement drafts in the same transaction.
