...
4. Optionally open or create a linked Billing Statement draft from the CSR draft. Existing billable item usages and service usages appear automatically, and later CSR draft usage changes synchronize to the linked statement. Repeating this action reopens an existing linked draft (preferring the one with the most CSR source lines) instead of making another empty draft. The statement remains unnumbered and cannot be finalized until this CSR is finalized. Deleting the CSR draft also removes its linked unnumbered Billing Statement drafts in the same transaction.
